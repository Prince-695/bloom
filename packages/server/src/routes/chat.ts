...
import { Mode, MessageStatus } from "@bloom/database/enums";
import {
...
// Strip every message and empty assistant message from the conversation
...
type InggestUsageForMessageParams = {
    messageId: string;
...
async function streamAIResponse(
    ...
    const persistInterruptedMessage = async () => {
        ...
        const elapseMs = Date.now() - startTime;
        ...
        return db.message.create({
            data: {
                ...
                mode,
                duration: Math.round(elapseMs / 1000),
            },
        });
    };

    const ingestUsageForMessage = async ({ messageId, status }: InggestUsageForMessageParams) => {
        if(!completedUsage) return;

        try {
            const billingUsage = calculateCreditsForUsage({
        ...
        for await (const part of result.fullStream) {
            ...
            if (part.type === "tool-result") {
                ...
                const tcPart = parts.find(
                    (p): p is Extract<MessagePart, { type: "tool-call" }> => p.type === "tool-call" && p.id === part.toolCallId
                );
